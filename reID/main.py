...
import reid.loss.CrossEntropyLabelSmooth;

import sys;


def get_data(dataname,dataVersion,argsCameraFilters, data_dir, height, width, batch_size, camstyle=0, re=0, workers=8):
    root = osp.join(data_dir, dataname)

    cameraFilters = argsCameraFilters.split(",");

    dataset = datasets.create(dataname,dataVersion,cameraFilters, root);
...
def main(args):
    ...
    dataset, num_classes, train_loader, query_loader, gallery_loader, camstyle_loader = \
        get_data(args.dataset,args.dataVersion,args.cameraFilters,args.data_dir, args.height,
    ...
    #criterion = reid.loss.CrossEntropyLabelSmooth.CrossEntropyLabelSmooth(num_classes).cuda();
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cameraFilters', type=str, default='c1,c2,c3,c4,c5,c6,c7,c8')
